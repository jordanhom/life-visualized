# Life Visualized Documentation Index

This page is the canonical navigation index for the project's durable
documentation.

## Start Here

1. [Project Overview](../01%20Project/PROJECT_OVERVIEW.md)
2. [Product Requirements](../06%20Product/PRODUCT_REQUIREMENTS.md)
3. [Architecture](../02%20Architecture/ARCHITECTURE.md)

## Canonical Sources

### Project

- [Project Overview](../01%20Project/PROJECT_OVERVIEW.md)

### Architecture

- [Architecture](../02%20Architecture/ARCHITECTURE.md)

### Domain Model

- [Domain Model](../03%20Domain%20Model/DOMAIN_MODEL.md)

### Core Workflows

- [Core Workflows](../04%20Core%20Workflows/CORE_WORKFLOWS.md)

### Operations

- [Operations](../05%20Operations/OPERATIONS.md)

### Product

- [Product Requirements](../06%20Product/PRODUCT_REQUIREMENTS.md)

### Execution

- [Execution Plan](../07%20Execution/EXECUTION_PLAN.md)

### AI Context

- [Repository Map](../08%20AI%20Context/REPO_MAP.md)

### Focused Module References

- [UI](../../ui.md)
- [Calculator](../../calculator.md)
- [Actuarial data](../../data.md)
- [Date utilities](../../dateUtils.md)
- [Grid renderer](../../gridRenderer.md)

### Historical Records

- Dated change summaries and the legacy memory-bank snapshot remain under `docs/` and `.kilocode/rules/memory-bank/`; they are evidence, not current execution guidance.

## Navigation Rules

- Read only the documents relevant to the current task.
- Treat linked canonical documents as authoritative over summaries.
- Add new documents to this index when they become durable sources of truth.
